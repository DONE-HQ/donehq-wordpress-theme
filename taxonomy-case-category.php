        ...
        <div class="container">
            <div class="common-hero__content">
                <div class="common-hero__title-wrap">
                    <h1 class="common-hero__title"><?php single_term_title(); ?></h1>
                </div>
                <div class="common-hero__frames">
                    <div class="cats-list">
                        <?php
                        // Получаем ID текущего термина таксономии
                        ...
                        if (!empty($categories) && !is_wp_error($categories))
                        {
                            ...
                            foreach ($categories as $category)
                            {
                                ...
                                // Проверяем, является ли текущий термин активным
                                $is_active = $current_term_id == $category->term_id ? ' active' : '';

                                // Выводим ссылку с учетом активного класса
                                echo '<a href="' . esc_url($category_link) . '" class="cat-link c-' . $counter . $is_active . '" data-category="' . esc_attr($category->slug) . '">' . esc_html($category->name) . '</a>';
